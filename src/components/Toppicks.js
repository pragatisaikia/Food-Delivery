...
const Toppicks = () => {
  return (
    <><div>
          <h1 className='hidden lg:block text-orange-500 font-bold text-3xl text-center py-2 '>Top Picks</h1>
      </div><div className='hidden lg:flex max-w-[1520px] m-auto py-2 px-2 '>
              <Splide options={{ perPage: 4, gap: "0.5rem", drag: 'free', arrows: false }}>
                  {topPicks.map((item) => (
                      <SplideSlide key={item.id}>
                          <div className='rounded-3xl relative'>
                              <div className='absolute w-full h-full bg-black/50 rounded-3xl text-white'>
                                  <p className='px-2 font-bold pt-4'>{item.title}</p>
